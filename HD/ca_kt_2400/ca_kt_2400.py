...
class KT():
    ...
    def keithley_init(self):
        ports_l = self.serial_ports()
        for port in ports_l:
            seri = serial.Serial(port, baudrate = 57600, bytesize = 8, stopbits = 1, parity = 'N')
            time.sleep(0.2)
            seri.write(':OUTP ON'+ '\r')
            print('Keithley OK')
            return seri
    ...
    def measure(self, ser, command):  # command D/S 414page
        if command == 1:
            meas1 = self.get_response(ser, ':FETCh?')  # no math function enabled just read

            print(meas1) #full meas          #D/S 353 page
            value = meas1[0:26]
    ...
    def set_system_key(self, ser, system_key):           # D/S 415 page
        #if system_key == 'DCV':
        ...
        while True:
            ...
            while repeat > 0:
                ...
                if mode == 0:
                    ...
                    wr.writerow([repeat,nowDate, nowTime, value, 'VDC'])
                elif mode == 2:
                    print("repeat:%s Date:%s Time:%s X:%s Y:%s lv:%s val:%.3f units:VDC " %(repeat,nowDate, nowTime, x, y, lv, value))
                    wr.writerow([repeat,nowDate, nowTime, x, y, lv, value, 'VDC'])
                repeat -= 1
            break
